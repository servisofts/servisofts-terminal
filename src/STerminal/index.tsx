...
export default class STerminal extends Component {
  scrollView;
  // input: TextInput;
  input;
  ...
  handlePressTab() {
    ...
    if (command.length > 0) {
      let opts = Object.keys(commands).filter(a => a.startsWith(command + ""))
  ...
  execute() {
    ...
    if (command) {
      ...
      const CMD = commands[cmd[0] + ""];
      if (CMD) {
        this.state.value = new CMD(this).execute(cmd)
      } else {
  ...
  newLine() {
    let promp = "servisofts@Servisofts-Terminal ~ % ";
